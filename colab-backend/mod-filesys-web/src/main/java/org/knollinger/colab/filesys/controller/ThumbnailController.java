...
{
    @Autowired()
    private IThumbnailService thumbSvc;

    @GetMapping(path = "/thumbs/{uuid}")
    public ResponseEntity<InputStreamResource> getThumb(@PathVariable("uuid") UUID uuid, //
        @RequestHeader(value = HttpHeaders.IF_NONE_MATCH, required = false) String ifNoneMatch)
    {
        try
        {
            BlobInfo blobInfo = this.thumbSvc.getThumbnail(uuid);
